...
! STATEOLD(32) : flag for inhomogeneity. Flag 1: normal DD-FEM coupled
! element that dislocations can glide hrough. Flag 0: dislocation nodes
! are fixed.
!
!
...
!	Define Initial state variables
		IF (TOTALTIME.EQ.0.0) THEN
			...
			IF (PROPS(3).eq.0.0) THEN
				STATEOLD(iblock,32)=0.0
			ELSE
				STATEOLD(iblock,32)=1.0
			END IF
...
!	Calculate stress and strain increment components in the crystal coordinate system
	...
	IF((TOTALTIME>=t_nextstep).AND.(t_nextstep>0.0)) THEN
		...
		DO iBlock=1,NBLOCK ! Stress output(FEMSTRESS) for ParaDiS
...
!			
			IF (surface_elements(Element_id(iBlock),2) .ge. 1) THEN ! Export the stress for all sufrace elements as Potential nucleation sites 
				IF (STATEOLD(iblock,14).EQ.1) THEN ! If surface elements are active for plastic deformation, we don't allowe nucleation.
...
!
!
	DOUBLE PRECISION, PARAMETER :: tolerance=1.001, burgers=1.0, shape_fn_tol=1.1, shape_fn_tol_rn=1.25, b_mag=2.864e-10
	DOUBLE PRECISION, PARAMETER :: PI=acos(-1.0_16)
...
!
	...
	DO i_segment=1, n_segment
		...
		DO iBlock = 1, nblock
			...
			IF (STATEOLD(iBlock,32).eq.0.0) GOTO 101 ! Element is impurity. No plastic defomration allowed
...
! Check the links and print out the element in which lies the segment center 
		...
		DO i_segment=1, n_segment_final
			...
			DO WHILE ((link_element.eq.0).and.(iBlock.le.nblock))
				IF (STATEOLD(iBlock,32).eq.0.0) GOTO 201 ! Element is impurity. No stress export to DD
...
!				link_flag(i_segment)=link_element
		...
		END DO
		CLOSE(16)
! Check the rn and fill rn_element matrix; COL(1): iBlock, COL(2): FEM element ID, COL(3): Element Flag (for node fixing)
		DO i_rn=1, num_rn
			iBlock=1
						...
						IF ((element_max(iBlock,3).ge.rn_nodes(i_rn,3)).and.(element_min(iBlock,3).le.rn_nodes(i_rn,3))) THEN
							IF(nnode.EQ.4) THEN
								...
								DO n=1,3
									eta(n)=invxv(n,1)*Ralra(1)+invxv(n,2)*Ralra(2)+invxv(n,3)*Ralra(3)
								END DO
								! Check if the dislocation node (rn) within shape function
								IF ((eta(1).ge.0.0).and.(eta(2).ge.0.0).and.(eta(3).ge.0.0)) THEN
							...
							ELSEIF(nnode.EQ.8) THEN
								...
								IF ((abs(eta(1)).le.shape_fn_tol_rn).and.(abs(eta(2)).le.shape_fn_tol_rn).and.(abs(eta(3)).le.shape_fn_tol_rn)) THEN
									rn_element(rn_id(i_rn,1)+1,1)=iBlock
									rn_element(rn_id(i_rn,1)+1,2)=jElem(iBlock)
									IF (STATEOLD(iBlock,32).eq.0.0) THEN ! The node is found inside the impurity
										rn_element(rn_id(i_rn,1)+1,3)=0 ! Change flag to 7 later to fix the node
									ELSE ! The node is found inside the maxtrix (allow dislocation motion)
										rn_element(rn_id(i_rn,1)+1,3)=1 
									END IF
								END IF
							END IF
						END IF			
...
! Update the density of each element
		IF(nnode.EQ.8) THEN ! At the moment, dislocation density update only works for C3D8 element
			DO iBlock=1, nblock
				STATEOLD(iBlock,31)=0.0 ! Reset density to zero
				DO j=1,8
					DO k=1,3
						xx(j,k)=Origin_CoordNode(iBlock,j,k)
					END DO
				END DO
				! Compute the volume of the finite element
				CALL CAL_element_volume(elem_volume, xx)
				element_volume(iBlock)=elem_volume
			END DO
			DO i_segment=1, n_segment_final ! Loop throughb all dislocation segment
				...
				IF ((segment_elem.gt.0).and.(segment_elem.le.nblock).and.(jElem(segment_elem).eq.link_elem_id(i_segment,2))) THEN 
					! The segment is found in this FEM subdomain				
					...
					END DO			
					! Determine the density contribution from segment center to rn1
...
! If a normal node (flagged 0) is found in side FEM domain, the coordinates is print out as three 0's to the output files
		...
		DO i_rn=1, num_rn
			...
			IF (rn_element(rn_id(i_rn,1)+1,1).gt.0) THEN ! node found inside FEM domain
				...
				! If a surface node (flagged 6) is found inside FEM domain, then we move the node back to its coordinate at the beginning of the time increment
					i_segment=1
					rn_surface_check=0
					...
					END DO				
				END IF
				IF (rn_element(rn_id(i_rn,1)+1,3).EQ.0) THEN ! the node is found inside the impurity
					rn_id(i_rn,2)=7 ! change node flag to 7 to fix the node
				END IF
			...
			ELSEIF (rn_element(rn_id(i_rn,1)+1,1).EQ.0) THEN ! node not found in FEM domain
				IF ((rn_id(i_rn,2).EQ.6)) THEN
				! If a surface node (flagged 6 found outside of FEM domain, we find the node's projection on the free surface.
					projection(1)=0.0
					projection(2)=0.0
					projection(3)=0.0
