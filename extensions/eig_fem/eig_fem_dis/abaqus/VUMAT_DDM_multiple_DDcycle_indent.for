...
!	
	...
	ELSE
		...
		IF (tim .le. 4.0) THEN
			ampValueNew = ampValueOld+5.0e-4			
		ELSE
...
!
	...
	DO iBlock = 1, nblock
		IF (element_within(iBlock) .EQ. 0) THEN
			STATEOLD(iblock,14)=0
		END IF
!       SDV32 stores the accumulated plastic strain e_yy        
		STATEOLD(iBlock,32)=STATEOLD(iBlock,32)+DT*STATEOLD(iBlock,16)
...
!	COMMON /sharedamp/ HOLD_ONE, HOLD_TWO
	...
	num_elements=i_Array(i_int_nTotalElements)-2
!
!--------------------------------------------------------------------
