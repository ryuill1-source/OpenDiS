...
! STATEOLD(33) : Damage variable
! STATEOLD(34) : Element deletion variable (default: 1, deletion: 0)
!
!
...
!	
	...
	ELSE
		...
		IF (tim .le. 0.0) THEN
			ampValueNew = ampValueOld+0.00028D0
		ELSE
...
!  
    ...
    DOUBLE PRECISION :: EMOD, ENU, c11, c12, c44, damage_model, critical_damage
	DOUBLE PRECISION :: deqps, Mises_Old, Mises_New, swapVal, hydroStress, triaxial
	DOUBLE PRECISION, DIMENSION(3) :: princ_Stress   
	DOUBLE PRECISION, DIMENSION(NBLOCK,3):: eigStress
...
!   Input Damage parameters
    damage_model=PROPS(3) ! 1: plastic strain, 2: plastic work, 3: Cockcroft & Latham
    critical_damage=PROPS(4)	
!
!====================================================================================
!
...
!
!	Compute princial stress values
 	Call vsprinc(NBLOCK, STRESSOLD, eigStress, NDIR, NSHR)
!==================================== ====================================
!                             DO  Nblock loop
!==================================== ====================================
    DO iblock=1,NBLOCK
...
!	Calculate stress and strain increment components in the crystal coordinate system
		...
		Mises_Old=sqrt(((StressGlobal(1,1)-StressGlobal(2,2))**2.0+(StressGlobal(2,2)-StressGlobal(3,3))**2.0+(StressGlobal(3,3)-StressGlobal(1,1))**2.0&
		+6.0*(StressGlobal(1,2)**2.0+StressGlobal(2,3)**2.0+StressGlobal(3,1)**2.0))/2.0)
		!------------------------------------ ------------------------------------
		...
		DO j=1,3
			...
			END DO
		END DO
		! Update equivalent plastic strain (based on von Mises yield function)
		deqps=DT*sqrt(2.0*(STATEOLD(iBlock,15)**2.0+STATEOLD(iBlock,16)**2.0+STATEOLD(iBlock,17)**2.0&
		+2.0*STATEOLD(iBlock,18)**2.0+2.0*STATEOLD(iBlock,19)**2.0+2.0*STATEOLD(iBlock,20)**2.0)/3.0)	
		!
		STATEOLD(iBlock,30)=STATEOLD(iBlock,30)+deqps
		! Update Mises Stress
		Mises_New=sqrt(((StressGlobal(1,1)-StressGlobal(2,2))**2.0+(StressGlobal(2,2)-StressGlobal(3,3))**2.0+(StressGlobal(3,3)-StressGlobal(1,1))**2.0&
		+6.0*(StressGlobal(1,2)**2.0+StressGlobal(2,3)**2.0+StressGlobal(3,1)**2.0))/2.0)		
		!
		! Update damage variable
		!
		IF (damage_model.eq.1.0) THEN ! Damage variable is plastic strain
			STATEOLD(iblock,33)=STATEOLD(iBlock,30)
		!
		ELSEIF (damage_model.eq.2.0) THEN ! Freudenthal model. Damage variable is plastic work
			STATEOLD(iblock,33)=STATEOLD(iblock,33)+0.5*(Mises_Old+Mises_New)*deqps
		!
		ELSEIF (damage_model.eq.3.0) THEN ! Cockcroft & Lathem model. Effect of Max. Tensile Stress on void growth
			! Find the principal stress in descending order
			princ_Stress(1)=eigStress(iblock,1)
			princ_Stress(2)=eigStress(iblock,2)
			princ_Stress(3)=eigStress(iblock,3)		
			IF (princ_Stress(1).lt.princ_Stress(2)) THEN
				swapVal=princ_Stress(1)
				princ_Stress(1)=princ_Stress(2)
				princ_Stress(2)=swapVal
			END IF
			IF (princ_Stress(2).lt.princ_Stress(3)) THEN
				swapVal=princ_Stress(2)
				princ_Stress(2)=princ_Stress(3)
				princ_Stress(3)=swapVal
			END IF		
			IF (princ_Stress(1).lt.princ_Stress(2)) THEN
				swapVal=princ_Stress(1)
				princ_Stress(1)=princ_Stress(2)
			...
			END IF	
			! Update damage variable
			IF ((Mises_Old.lt.1.0e-3).or.(princ_Stress(1).lt.0.0)) THEN
				STATEOLD(iblock,33)=STATEOLD(iblock,33)
			ELSE
				STATEOLD(iblock,33)=STATEOLD(iblock,33)+(princ_Stress(1)/(Mises_Old))*deqps
			END IF
			!
		ELSEIF (damage_model.eq.4.0) THEN ! Rice & Tracy void growth model. Exponential relation between void growth and triaxiality
			! Caculate stress triaxiality
			hydroStress=(1/3)*(StressGlobal(1,1)+StressGlobal(2,2)+StressGlobal(3,3))
			triaxial=(3.0*hydroStress)/Mises_New
			IF (Mises_New.lt.1.0e-3) THEN
				STATEOLD(iblock,33)=STATEOLD(iblock,33)
			ELSE
				STATEOLD(iblock,33)=STATEOLD(iblock,33)+(exp(triaxial))*deqps
			END IF
		ELSEIF (damage_model.eq.5.0) THEN ! Damage variable is dislocation density
			STATEOLD(iblock,33)=STATEOLD(iBlock,31)
			!						
		END IF
		! Element deletion
		IF (STATEOLD(iblock,33).GT.critical_damage) THEN
			STATEOLD(iblock,34)=0
		END IF			
		!
		...
		STATENEW(iBlock,33)=STATEOLD(iBlock,33)
		STATENEW(iBlock,34)=STATEOLD(iBlock,34)
		!
    END DO
	!
...
!
	IF (TOTALTIME.EQ.0.0) THEN
		DO iBlock = 1, nblock
			...
			STATEOLD(iBlock,33)=0.0
			STATEOLD(iBlock,34)=1
		END DO
	END IF
...
! Check the links and print out the element in which lies the segment center 
		...
		DO i_segment=1, n_segment_final
			...
			DO WHILE ((link_element.eq.0).and.(iBlock.le.nblock))
				IF (STATEOLD(iblock,34).EQ.1) THEN
