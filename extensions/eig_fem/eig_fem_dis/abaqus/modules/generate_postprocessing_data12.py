...
def generate_postprocessing_data12(Num_Elements,Element_type,Elements,Elements_Center,Elements_Min,Elements_Max,
    ...
    for i in range(num_cpus):
        ...
        if i==0:
            ...
            #(25/06/26, kyeongmi) np.ndarray doesn't have 'extend' 
            Stress_int_DDM = np.vstack([Stress_int_DDM, Stress_int_point_temp])

    double_write=0
    if len(Stress_int_DDM)>Num_Elements:
        double_write=1
    ...
    for i in range(num_cpus):
        ...
        if double_write:       # The output from ABAQUS is doubly written in this cycle
            ...
            #(25/06/26 kyeongmi) slicing number should be integer type
            size_stress = int(len(Stress_int_point_temp)/2)
            
            #Stress_int_point_temp[:size_stress]
            #(25/06/26 kyeongmi) 2D slicing, not 1D
    ...
    fileID = open(os.path.join(foldername_FEMstress,filename_stress_nucleation), 'w')
    ...
    for i in range(length_rn):
        if rn[i,3]==0:  # flag-0 node
            ...
            while j<rn_check_length and rn[i,3]==6:
                if rn[i,4]==rn_check[j,0]:   # Find a match from ABAQUS output
                    if abs(rn_check[j,2])>0 or abs(rn_check[j,3])>0 or abs(rn_check[j,4])>0:
                        rn[i,0]=rn_check[j,2]
                        rn[i,1]=rn_check[j,3]
                        rn[i,2]=rn_check[j,4]
                    rn_check=np.delete(rn_check,j,axis=0)
...
## Re-arrange rn, links, and segment, then remove rn, links that are outside of the domain
    if n_step>=1 and length_rn>1:
        ...
        # 
        write_node_data4(os.path.join(foldername_restart,'restart'), rn, links, xyzlimit)
        #
    else:
    ...
    fileID=open(os.path.join(foldername_FEMstress, filename_FEMstress_head),'w')
    z=0
    uni_stress=500    
    if length_rn>1:
        for i in range(length_links):
