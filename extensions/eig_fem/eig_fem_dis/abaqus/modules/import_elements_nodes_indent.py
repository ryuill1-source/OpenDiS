...
def import_elements_nodes_indent(foldername_tests, foldername_Job,
                                 foldername_ABAQUS,
                                 jobname_head, abaqus_input_filename,
                                 surface_element_set_name,
                                 element_type, format_spec_elset, elset_col_num):
    """
    Parse ABAQUS .inp file to extract nodes, elements, and surface element sets.
    Indenter (ID=1) is added as virtual element in the output.
    Returns a dict with:
    ...
    # Write OriginalCoordNodes.txt (with indenter)
    
    out_path = os.path.join('..', '..',
                            foldername_tests, foldername_Job,
                            foldername_ABAQUS, 'OriginalCoordNodes.txt')
    with open(out_path, 'w') as f:
            # First row: virtual indenter (ID 1, all zeros)
            f.write('1 ' + '0.0 ' * 24 + '\n')
            for j in range(Num_Elements):
                eid = int(raw_elems[j, 0]) + 1  # +1 for virtual indenter
